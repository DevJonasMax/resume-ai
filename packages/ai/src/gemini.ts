...
export class GeminiProvider implements AIProvider {
	...
	constructor(apiKey?: string, modelName?: string) {
		...
		if (!key) {
			throw new Error(
				"GEMINI_API_KEY is required to instantiate GeminiProvider",
			);
		}
		...
		this.defaultModel = NON_DEPRECATED_GEMINI_MODELS.includes(
	...
	public async generateStructured<T>(options: {
		...
		try {
			...
			throw new Error(
				`Gemini Provider Structured Generation Failure (${targetModel}): ${detailed}`,
				{
					cause: err,
				},
			);
	...
	public async generateText(options: {
		...
		try {
			...
			const detailed = extractDetailedErrorMessage(err);
			throw new Error(
				`Gemini Provider Text Generation Failure (${targetModel}): ${detailed}`,
				{
					cause: err,
				},
			);
		...
		try {
			...
			async function* iterateStream() {
				try {
					...
					throw new Error(
						`Gemini Provider Streaming Iteration Failure (${targetModel}): ${detailed}`,
						{
							cause: streamErr,
						},
					);
			...
			const detailed = extractDetailedErrorMessage(err);
			throw new Error(
				`Gemini Provider Streaming Failure (${targetModel}): ${detailed}`,
				{ cause: err },
			);
		}
	}
