      ...
      <div className="flex flex-col h-[calc(100vh-140px)]">
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto pb-1.5 no-scrollbar">
            {QUICK_PROMPT_SUGGESTIONS.map((suggestion, idx) => (
              <button
                ...
                disabled={isRefining}
                className="whitespace-nowrap px-2.5 py-1 bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 rounded-full text-xs text-zinc-300 hover:text-white transition-all cursor-pointer disabled:opacity-50 shrink-0"
              >
                {suggestion}
              </button>
            ))}
        ...
        <div className="relative flex-1 overflow-hidden">
          ...
          >
                ...
                >
                  ...
                  <div
                    className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 text-white ${
                      isUser
                        ...
                        : "bg-indigo-600 border border-indigo-500 shadow-md shadow-indigo-600/30"
                    }`}
                  >
                    {isUser ? (
                  ...
                  <div
                    className={`flex flex-col gap-1 max-w-[85%] rounded-xl p-3.5 ${
                      isUser
                        ? "bg-indigo-950/40 text-zinc-100 border border-indigo-800/50"
                        : "bg-zinc-900/80 text-zinc-200 border border-zinc-800 shadow-sm"
                    }`}
                  >
            ...
            {isRefining && (
              <div className="flex items-center gap-3 text-xs text-zinc-400 p-3 bg-zinc-900/50 rounded-xl border border-zinc-800/80">
          ...
          {showJumpButton && (
            <button
              ...
              className="absolute bottom-3 left-1/2 -translate-x-1/2 flex items-center gap-1.5 px-3 py-1.5 bg-zinc-900/95 hover:bg-zinc-800 text-white rounded-full text-xs font-medium border border-zinc-700 shadow-xl transition-all duration-200 cursor-pointer animate-in fade-in slide-in-from-bottom-2"
            >
        ...
        <div className="pt-3 border-t border-zinc-800">
          <div className="relative rounded-xl border border-zinc-800 bg-zinc-900/70 p-2 focus-within:border-indigo-500/80 focus-within:ring-1 focus-within:ring-indigo-500/50 transition-all">
            <textarea
              value={inputPrompt}
