				...
				if (trimmed.startsWith("• ") || trimmed.startsWith("* ")) {
					...
					return (
						<div key={lineIdx} className="flex items-start gap-2 pl-1">
							<span className="text-[#d8b4fe] leading-5">•</span>
		...
		<div className="w-full rounded-2xl bg-rose-950/25 border border-rose-500/35 p-4 shadow-lg shadow-rose-950/20 text-xs space-y-3 animate-in fade-in duration-200">
			...
			{showDetails && (
				<div className="p-2.5 rounded-lg bg-black/40 border border-zinc-800 text-[10px] text-zinc-400 font-mono space-y-1">
					<div>
						Model: <span className="text-zinc-200">{activeModel}</span>
					</div>
					<div>
						Timestamp: <span className="text-zinc-200">{timestamp}</span>
					</div>
...
export function AiChatDock() {
		...
		return (
			...
			>
				<div
					...
					className="flex items-center gap-3 px-3.5 py-2 rounded-full bg-[#121417]/90 hover:bg-[#16191f]/95 backdrop-blur-xl border border-[rgba(255,255,255,0.12)] hover:border-[#d8b4fe]/40 shadow-[0_12px_36px_rgba(0,0,0,0.5),0_0_15px_rgba(216,180,254,0.06)] transition-all duration-200 cursor-pointer group active:scale-95"
				>
	...
	return (
		...
		>
			...
			<header className="flex items-center justify-between px-4 py-3 border-b border-[rgba(255,255,255,0.08)] bg-[#15181d]/80">
			...
			{isMockActive && (
				<div className="px-3.5 py-1.5 bg-amber-500/10 border-b border-amber-500/20 flex items-center justify-between text-[11px] text-amber-200">
					<span className="flex items-center gap-1.5">
						<span>⚠️</span>
						<span><strong>Mock Mode Active:</strong> No live Gemini API key detected. Responses are simulated offline templates.</span>
					</span>
				</div>
			)}
								...
								return (
									<div key={msg.id} className="w-full">
										<RealErrorCard
											...
											onRetry={
												lastPromptSent ? handleRetryLastPrompt : undefined
											}
										/>
									</div>
								);
							...
							return (
								...
								>
									...
									{isUser ? (
										...
										<AgentAvatar seed={agentSeed} size={28} animated={false} />
									)}
									...
									>
										<div className="flex items-center justify-between gap-4 text-[10px] text-zinc-400 mb-0.5">
						...
						{isRefining && (
							<div className="flex flex-col gap-2 p-3 bg-[#181b1f]/80 rounded-xl border border-[rgba(216,180,254,0.2)]">
								<AIReasoning isStreaming={true} defaultOpen={true}>
									Evaluating ATS keyword distribution, aligning bullet points
									with verified candidate experience, and executing structured
									refinement via {selectedModel}...
								</AIReasoning>
								<div className="flex items-center gap-2 text-[10px] text-zinc-400 font-mono pl-1">
									...
									/>
									<span>
										Synthesizing tailored document &amp; updating LaTeX
										preview...
									</span>
								</div>
							</div>
