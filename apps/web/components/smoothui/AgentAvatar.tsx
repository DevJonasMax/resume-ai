...
> & {
  ...
  animated?: boolean;
  /** Optional status indicator state */
  state?: "idle" | "thinking" | "generating" | "error";
...
export const AgentAvatar = ({
  ...
  useEffect(() => {
    ...
    const hash = hashSeed(seed || "default-agent");
  ...
  return (
    <div className={cn("relative inline-flex items-center justify-center shrink-0", className)}>
      ...
      {showStatusIndicator && (
        <span
          className={cn(
            "absolute -bottom-0.5 -right-0.5 rounded-full ring-2 ring-[#0c0d0e] shadow-xs",
            size >= 48 ? "w-2.5 h-2.5" : "w-2 h-2",
            state === "error"
              ? "bg-rose-500"
