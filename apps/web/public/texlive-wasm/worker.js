...
`;
    ...
    class WorkerImpl {
        ...
        async init(opts, host) {
            ...
            this.glueFactory = (await importModule(glueUrl).then(async (m)=>{
                await m.__tla;
                return m;
            })).default;
        ...
        async createInstance() {
            ...
            if (this.engineId !== "biber") {
                ...
                mkdirCached(FS, "/texmf-dist/web2c/pdftex", dirs);
                if (!pathExists(FS, "/texmf-dist/web2c/pdftex/pdflatex.fmt")) {
                    const fmtUrl = this.config?.fmtPath || "/texlive-wasm/texmf/web2c/pdftex/pdflatex.fmt";
                    try {
                        const r = await fetch(fmtUrl);
                        if (r.ok) {
                            const fmtBytes = new Uint8Array(await r.arrayBuffer());
                            FS.writeFile("/texmf-dist/web2c/pdftex/pdflatex.fmt", fmtBytes);
                        }
                    } catch (e) {
                        console.warn("Could not fetch fmt file:", e);
                    }
                }
...
${this.stderrBuf}`);
    ...
    function mkdirCached(FS, path, seen) {
        ...
        try {
            if (FS.mkdirTree) {
                FS.mkdirTree(path);
            } else if (!pathExists(FS, path)) {
                const parent = dirname(path);
                if (parent !== path) mkdirCached(FS, parent, seen);
                FS.mkdir(path);
            }
        } catch (e) {
            // Ignore existing or hierarchy errors in WASMFS
        }
        seen.add(path);
    }
